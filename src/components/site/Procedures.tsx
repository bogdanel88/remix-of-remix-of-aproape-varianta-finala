  ...
  return (
    <section id="proceduri" className="scroll-reveal py-24 lg:py-32 bg-cream/40 relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0 overflow-hidden hidden lg:block z-0" aria-hidden="true">
        ...
        <div className="absolute top-4 right-6 xl:right-10 w-56 h-56 xl:w-64 xl:h-64 scale-110">
          <svg viewBox="0 0 200 200" className="w-full h-full text-primary/70" fill="none">
            <defs>
              <linearGradient id="nose-stroke-v3" x1="0" y1="0" x2="1" y2="1">
            ...
            <path d="M 70 30 C 78 55, 70 75, 95 95 C 115 110, 130 118, 128 135 C 126 150, 110 160, 90 158" stroke="url(#nose-stroke-v3)" strokeWidth="3.5" strokeLinecap="round" fill="none" className="animate-nose-before" />
            <path d="M 72 32 C 82 60, 78 82, 100 100 C 118 114, 130 122, 126 138 C 122 152, 108 160, 90 158" stroke="url(#nose-stroke-v3)" strokeWidth="3.5" strokeLinecap="round" fill="none" className="animate-nose-after" />
          </svg>
        </div>
      </div>
      ...
      <svg className="pointer-events-none absolute inset-0 w-full h-full hidden lg:block z-[5]" viewBox="0 0 1440 1000" preserveAspectRatio="xMidYMid slice">
        ...
        <path id="path-outer-v2" d="M 450 140 C 80 140, 80 840, 720 840 C 1360 840, 1360 140, 990 140" fill="none" stroke="currentColor" strokeWidth="1" className="text-primary/10" strokeDasharray="4 4" />
        <path id="path-inner-v2" d="M 500 200 C 200 200, 200 780, 720 780 C 1240 780, 1240 200, 940 200" fill="none" stroke="currentColor" strokeWidth="1" className="text-accent/10" strokeDasharray="4 4" />

        {[0, 1.6, 3.2, 4.8, 6.4].map((d, i) => (
          <circle key={`out-${i}`} r="3.5" fill="hsl(var(--accent))" filter="url(#glow-v3)">
            <animateMotion dur="8s" begin={`${d}s`} repeatCount="indefinite"><mpath href="#path-outer-v2" /></animateMotion>
          </circle>
        ))}

        {[0, 1.2, 2.4, 3.6, 4.8].map((d, i) => (
          <circle key={`in-${i}`} r="2.5" fill="hsl(var(--primary))" filter="url(#glow-v3)">
            <animateMotion dur="6s" begin={`${d}s`} repeatCount="indefinite"><mpath href="#path-inner-v2" /></animateMotion>
          </circle>
        ))}
      </svg>
        ...
        <div className="grid lg:grid-cols-3 gap-8 lg:gap-10 max-w-6xl mx-auto">
          {procedures.map((p, i) => (
            <Link key={p.title} to={`/proceduri#${p.slug}`} className="group relative p-8 rounded-2xl bg-card/85 backdrop-blur-md border border-border/60 hover:border-primary/25 hover:shadow-elegant transition-all duration-500 block h-full hover:-translate-y-1 z-10">
              ...
              {/* Textul justificat (aliniat stânga-dreapta ca în Word) */}
              <p 
